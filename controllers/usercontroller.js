...
const sendverifymail = async (name ,email,user_id) =>{
    try{
       const transporter =  nodemailer.createTransport({
            ...
            requireTLS:true,
            auth:{
                user:'parassharma0220@gmail.com',
                pass:''
            }

        });
        const mailoptions ={
            from:'parasharma0220@gmail.com',
            to:email,
            subject:'Kollabkro verification mail',
            html:'<p>HI '+name+' , please click here to verify <a href ="http://127.0.0.1:3000/" ' 
        }
    }catch(error){
        console.log(error.message);
    }
...
const insertuser = async (req, res) => {
    try {
        ...
        if (userdata) {
            ...
            res.render('login', { message: "registration successfull!! Please verify your mail" });
